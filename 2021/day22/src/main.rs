...
impl Cube {
    ...
    fn get_intersection(&self, other: &Cube) -> Cube {
        let (sx,sy,sz) = self.start;
        let (ex,ey,ez) = self.end;
        let (si,sj,sk) = other.start;
        let (ei,ej,ek) = other.end;
        Cube{
            start: (max(sx,si), max(sy,sj), max(sz,sk)),
            end: (min(ex,ei), min(ey,ej), min(ez,ek)),
        }
    }
...
impl Mass {
    fn new() -> Mass {Mass{volume: vec![], void: vec![]}}

    fn add(&mut self, other: Cube) {
        let mut neg = vec![];

        for cube in self.volume.iter() {
            if cube.intersects_with(&other) {
                if cube.contains(&other) {return}

                let n = cube.get_intersection(&other);
                neg.push(n);
            }
        }
        self.volume.push(other);
        self.void.append(&mut neg);
    }
    ...
    fn total_volume(&self) -> isize {
        ...
        let mut voids = self.void.clone();
        let mut total = 0;

        println!("volumes: {}, voids: {}", volumes.len(), voids.len());

        while volumes.len() > 0 && voids.len() > 0 {
            // sum up volumes
            for c in volumes.iter() {
                total += c.volume() as isize;
            }
            ...
            // find negative space overlaps
            let mut m1 = Mass::new();
            for &mut c in voids.iter_mut() {
                m1.add(c);
            }

            let mut m2 = Mass::new();
            for &mut c in m1.void.iter_mut() {
                m2.add(c);
            }
